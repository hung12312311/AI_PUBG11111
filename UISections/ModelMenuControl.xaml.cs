...
namespace Aimmy2.Controls
{
    public partial class ModelMenuControl : UserControl
    {
        ...
        public ListBox Slot1ModelListBoxControl => Slot1ModelListBox;
        public Label Slot1NotifierControl => Slot1Notifier;
        ...
        private async Task LoadStoreMenuAsync()
        {
            ...
            catch (Exception e)
            {
                await Dispatcher.InvokeAsync(() =>
                {
                    new NoticeBar($"Lỗi tải cửa hàng: {e.Message}", 10000).Show();
        ...
        private void OpenFolderB_Click(object sender, RoutedEventArgs e)
        {
            if (sender is Button clickedButton && clickedButton.Tag != null)
            {
                try
                {
                    ...
                    {
                        new NoticeBar($"Không tìm thấy thư mục: {path}", 5000).Show();
                    }
                }
                catch (Exception ex)
                {
                    new NoticeBar($"Không mở được thư mục: {ex.Message}", 5000).Show();
                }
            }
        }
        ...
        private void Slot1ModelSearchBox_TextChanged(object sender, TextChangedEventArgs e)
        {
            FilterListBoxBySearch(Slot1ModelListBox, (TextBox)sender);
        }
        ...
        private void ModelListBox_DragOver(object sender, DragEventArgs e)
        {
            ...
            if (e.Data.GetDataPresent(DataFormats.FileDrop))
            {
                ...
                bool valid = files.All(f => {
                    ...
                    return ext == ".onnx" || ext == ".engine" || ext == ".trt";
                });
            ...
            if (e.Data.GetDataPresent(DataFormats.FileDrop))
            {
                ...
                foreach (string file in files)
                {
                    ...
                    if (File.Exists(destPath))
                    {
                        ...
                        new NoticeBar($"File trùng, đã đổi tên: {Path.GetFileName(duplicatePath)}", 3000).Show();
                        continue;
                    }
                    try
                    {
                        ...
                        new NoticeBar($"Đã chuyển model: {Path.GetFileName(destPath)}", 3000).Show();
                    }
                    catch (Exception ex)
                    {
                        new NoticeBar($"Lỗi di chuyển model: {ex.Message}", 5000).Show();
                    }
                }
            }
        }
            ...
            if (e.Data.GetDataPresent(DataFormats.FileDrop))
            {
                ...
                foreach (string file in files)
                {
                    ...
                    try
                    {
                        ...
                        new NoticeBar($"Đã chuyển config: {Path.GetFileName(destPath)}", 3000).Show();
                    }
                    catch (Exception ex)
                    {
                        new NoticeBar($"Lỗi di chuyển config: {ex.Message}", 5000).Show();
                    }
                }
            }
        }
